...
- The 24 September read-only check found a daily database-backup script under `/usr/local/sbin/backup-fresh-app-databases`, using `/var/backups/fresh-app-databases` and a `-mtime +6` rotation rule. A separate `/srv/t2q/backups` inventory contains 84 files, including seven database dumps and deployment/configuration snapshots. This directory's file modification dates span 23 August–20 September; those dates do not establish individual account-deletion dates. No backup contents were read and no files were deleted. The separate archives and restore-time deletion handling remain unverified.
