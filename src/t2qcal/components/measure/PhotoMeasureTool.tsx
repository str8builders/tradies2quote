...
import {REFERENCE_LENGTHS,angleAtVertex,formatArea,formatDegrees,formatMm,measuredMm,pixelDistance,polygonAreaMm2,quadSidesMm,scaleFromReference} from "@/t2qcal/lib/measure";
...
export function PhotoMeasureTool(){
  ...
  const draw=useCallback(()=>{
    ...
    if(reference.length===2){seg(reference[0],reference[1],YELLOW);dot(reference[0],YELLOW);dot(reference[1],YELLOW);const m=S({x:(reference[0].x+reference[1].x)/2,y:(reference[0].y+reference[1].y)/2});tag(`ref ${formatMm(knownMm,unit)}`,m.x,m.y-11,YELLOW);}
    else if(reference.length===1)dot(reference[0],YELLOW);
    for(const [a,b] of lengths){seg(a,b,ORANGE);dot(a,ORANGE);dot(b,ORANGE);const m=S({x:(a.x+b.x)/2,y:(a.y+b.y)/2});tag(scale?formatMm(measuredMm(a,b,scale),unit):"set reference",m.x,m.y-11,ORANGE);}
    for(const poly of shapes){ctx.beginPath();poly.forEach((p,i)=>{const P=S(p);if(i===0)ctx.moveTo(P.x,P.y);else ctx.lineTo(P.x,P.y);});ctx.closePath();ctx.fillStyle="rgba(255,95,21,.18)";ctx.fill();ctx.lineWidth=1.5;ctx.strokeStyle=ORANGE;ctx.stroke();poly.forEach(p=>dot(p,ORANGE));const c=poly.reduce((acc,p)=>({x:acc.x+p.x/poly.length,y:acc.y+p.y/poly.length}),{x:0,y:0});const C=S(c);tag(scale?formatArea(polygonAreaMm2(poly,scale),unit):"set reference",C.x,C.y,ORANGE);}
    for(const [a,v,b] of angles){seg(v,a,ORANGE);seg(v,b,ORANGE);dot(a,ORANGE);dot(v,YELLOW);dot(b,ORANGE);const V=S(v);tag(formatDegrees(angleAtVertex(a,v,b)),V.x,V.y-16,YELLOW);}
  ...
  },[layout,reference,lengths,shapes,angles,draft,mode,scale,knownMm,unit,drag]);
  ...
  const lastQuad=scale?[...shapes].reverse().map(p=>quadSidesMm(p,scale)).find((q):q is [number,number]=>q!==null)??null:null;
  const lastAngle=angles.length?angleAtVertex(...angles[angles.length-1]):null;
  const stepText=!hasImage?"Take a photo or choose one from your phone."
    :reference.length<2?`Tap both ends of the reference: ${refId==="custom"?(customMm?`${customMm} mm`:"enter its length below"):REFERENCE_LENGTHS.find(r=>r.id===refId)?.label}.`
  ...
  return <section className="measure-tool" data-testid="measure-photo">
    ...
    {(lengths.length>0||shapes.length>0||angles.length>0)&&<ol className="measure-list" aria-label="Measurements" data-testid="measure-photo-results">
      {lengths.map(([a,b],i)=><li key={`l${i}`}><span>L{i+1}</span><strong>{scale?formatMm(measuredMm(a,b,scale),unit):"—"}</strong><small>{Math.round(pixelDistance(a,b))} px</small></li>)}
      {shapes.map((poly,i)=><li key={`a${i}`}><span>A{i+1}</span><strong>{scale?formatArea(polygonAreaMm2(poly,scale),unit):"—"}</strong><small>{poly.length} corners{scale&&quadSidesMm(poly,scale)?` · ${formatMm(quadSidesMm(poly,scale)![0],unit)} × ${formatMm(quadSidesMm(poly,scale)![1],unit)}`:""}</small></li>)}
      {angles.map(([a,v,b],i)=><li key={`g${i}`}><span>∠{i+1}</span><strong>{formatDegrees(angleAtVertex(a,v,b))}</strong><small>at the corner</small></li>)}
    </ol>}
