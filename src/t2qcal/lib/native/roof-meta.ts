 ...
 * exact port of `Field.sanitizedDisplayValue` so a ported compute sees the same
 * clamped, display-unit values the Swift `Tool.init` wrapper hands its body.
 *
 * Shared by the roof and drainage ports; both categories own `native/roof-*.ts`
 * only, so this lives here rather than in a common module.
 */
...
/** The native `Tool` record for a slug. */
export function nativeTool(slug: string): MetaTool {
 ...
 * catalogue. The trailing 3D sheet is dropped: the web picker appends the
 * assembly itself from `showsAssembly`/`assembly`, exactly as
 * `Tool.drawingSheets` does natively.
 */
export function nativeShell(slug: string, kind: (native: string) => DiagramKind = (k) => k as DiagramKind) {
...
/**
 * Exact port of the sanitising wrapper in `Tool.init`: every entered value is
 * replaced by its default when non-finite, rounded when the field is a count,
 * and clamped to the field range expressed in the display unit.
 */
...
/** `ord` — the two-digit running index used by the native set-out lists. */
