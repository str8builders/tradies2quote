...
export function attachToActiveJob(store:Store,calculationId:string):string|null{if(!isUUID(calculationId))throw new Error("Invalid calculation reference.");const data=readDeviceJobs(store),job=data.jobs.find(j=>j.id===data.activeId);if(!job)return null;if(!job.calculationIds.includes(calculationId))job.calculationIds.push(calculationId);store.setItem(KEY,JSON.stringify(data));return job.name;}
