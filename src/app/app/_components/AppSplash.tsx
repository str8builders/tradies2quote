...
export default function AppSplash({ storageKey = "t2q-welcome-v2", tagline = "Less paperwork. More time on the tools.", holdMs = 4700 }: {
  ...
  return <dialog open ref={dialogRef} onCancel={(event) => { event.preventDefault(); close(); }}
    ...
    <div className="relative flex min-h-full flex-col items-center justify-center overflow-clip px-6 py-10 text-center"
      ...
      <div aria-hidden="true" className="w-full max-w-[460px]">{play ? <WelcomePlayer /> : <div style={{ aspectRatio: "720 / 520" }} />}</div>
