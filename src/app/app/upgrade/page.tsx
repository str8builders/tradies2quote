...
export default async function UpgradePage({searchParams}:{searchParams:Promise<{stripe?:string;plan?:string;from?:string}>}){
 ...
 const nativeFallback=<main data-legacy-body="" className="mx-auto max-w-3xl px-4 py-14"><h1 className="text-3xl font-semibold">{sub.state==='expired'?NEW_QUOTES_PAUSED:'Account information'}</h1><p className="mt-4 text-sm text-ink-300">You can still view, send and download your existing quotes and invoices.</p></main>;
 ...
 return <div className="min-h-screen text-white"><AppHeader context="Plans"/><HideInNativeApp fallback={nativeFallback}><main data-legacy-body="" className="mx-auto max-w-6xl px-4 py-10 sm:px-6"><header className="t2q-page-intro"><div className="t2q-section-label-pro">{"// room to grow"}</div><h1 className="mt-3 text-3xl font-semibold tracking-tight sm:text-4xl">A plan for the way you work.</h1><p className="mt-4 max-w-2xl text-sm leading-relaxed text-ink-300">{sub.state==='expired'?'Your trial has ended. Choose a plan to create new quotes. Existing quotes remain available.':'Start on your own, or bring your crew. One monthly price, including GST. Cancel anytime.'}</p></header>
