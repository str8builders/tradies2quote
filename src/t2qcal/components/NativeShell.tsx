...
export function NativeShell(){
  ...
  return <><div className="native-navigation-header">
    ...
    <div className="native-header-actions">{!tool&&<a className="native-round" href="/t2qcal/calculators#search" aria-label="Search calculators"><MagnifyingGlass size={22}/></a>}<button className="native-round" onClick={()=>dialog.current?.showModal()} aria-label="Settings"><User size={22}/></button></div>
  </header></div>
  <dialog ref={dialog} aria-label="Settings" className="native-settings"><div className="native-sheet-heading"><h2>Settings</h2><button onClick={()=>dialog.current?.close()} aria-label="Close settings"><X size={24}/></button></div><div className="native-group"><a className="native-row" href="/login?next=%2Ft2qcal%2Fcalculators"><User size={26}/><span><strong>Sign in with Tradies2Quote</strong><small>Use your account to save working across devices.</small></span></a><a className="native-row" href="/app"><span><strong>Open Tradies2Quote</strong><small>Quotes, invoices, clients and materials.</small></span></a><a className="native-row" href="/t2qcal/device"><span><strong>Saved on this device</strong><small>Open, export or restore your calculations.</small></span></a><a className="native-row" href="/t2qcal/install"><span><strong>Install T2QCAL</strong><small>Add this app to your Home Screen.</small></span></a></div><WebAppControls/><p className="native-footnote">T2QCAL · Construction calculators</p></dialog></>;
}
